...
func EnsureLongDeviceId(shortDeviceId string) (longDeviceId string, err error) {
	...
	if strings.HasPrefix(shortDeviceId, common.DEVICE_PREFIX) {
	...
	longDeviceId = common.DEVICE_PREFIX + uuidStr
...
func ShortId(longId string) (shortId string, err error) {
	...
	if !strings.HasPrefix(longId, common.DEVICE_PREFIX) {
		return "", errors.New("expected " + common.DEVICE_PREFIX + " as prefix")
	}
